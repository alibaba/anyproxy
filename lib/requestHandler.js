...
function userRequestHandler(req,userRes){
    ...
    */
    ...
    if (global.auth && protocol === 'http' && auth(req, userRes, resourceInfo, resourceInfoId) === false) {
        return ;
    }
...
function connectReqHandler(req, socket, head){
    ...
    if (global.auth && auth(req, socket) === false) {
        return ;
    }
...
// proxy authorization
function auth(req, res, info = null, id = null) {
    var index, header = req.headers["proxy-authorization"],
        end = function (code) {
            if (info === null) {
                if (global.recorder) {
                    global.recorder.appendRecord({
