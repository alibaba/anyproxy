...
function fetchRemoteResponse(protocol, options, reqData, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const proxyReq = (/https/i.test(protocol) ? https : http).request(options, (res) => {
      const originHeader = res.headers;
      ...
      const finishCollecting = () => {
        new Promise((fulfill, rejectParsing) => {
          ...
          resolve({
            statusCode,
            originHeader, // https://github.com/alibaba/anyproxy/issues/216
            header: resHeader,
            body: serverResData,
            _res: res,
          });
      ...
      if (ifServerGzipped || isServerDeflated) {
  ...
  return function (req, userRes) {
    ...
    */
    const sendFinalResponse = (finalResponseData) => {
      ...
      // if there is neither content-length nor transfer-encoding, set the content-length
      if (!global._throttle
        ...
        if (userConfig.response) {
          ...
          return {
            response: {
              ...
              originHeader: remoteResponse.originHeader
            },
