    ...
    */
    ...
    function dealWithRemoteResonse(callback){
        ...
        var proxyReq = ( /https/.test(protocol) ? https : http).request(options, function(res) {
            ...
            try {
                // for some requests, here may throw error like: `Fatal error: The header content contains invalid characters`
                userRes.writeHead(statusCode, resHeader);
            } catch (e) {
                logUtil.printLog("err write head :" + require('util').inspect(resHeader) + "  " + req.url, logUtil.T_ERR);
            }
