  ...
  try {
    ...
    */
    ...
    proxyWs.onmessage = (event) => {
      ...
      userRule.beforeReadWsMessage && userRule.beforeReadWsMessage(event, wsUrl);
    ...
    wsClient.onmessage = (event) => {
      ...
      userRule.beforeSendWsMessage && userRule.beforeSendWsMessage(event, wsUrl);
      sendProxyMessage(event);
    }

    wsClient.onclose = (event) => {
      logUtil.debug(`original ws closed with code: ${event.code} and reason: ${event.reason}`);
      const targetCloseInfo = getCloseFromOriginEvent(event, wsUrl);
