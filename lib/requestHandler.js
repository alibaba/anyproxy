...
const error502PugFn = pug.compileFile(__dirname + '/customPages/502.pug');


let internalHttpsPort; // internal https port
let httpsServerMgrInstance; // internal https server instance
...
function userRequestHandler(req,userRes){
    ...
    */
    ...
    logUtil.printLog(color.green("received request to : " + host + path));
    ...
    function routeReq(callback){
        ...
        .catch(error => {
            logUtil.printLog("error happend in co for shouldUseLocalResponse':"
    ...
    function dealWithLocalResponse(callback){
        co(function * () {
            const responseData = yield userRule.dealLocalResponse(req,reqData);
            ...
            // Ensure both statusCode and header are returned
            if (!statusCode || !resHeader) {
                throw new Error('Status code and header are required when responsed in `dealLocalResponse()`');
            }
        ...
        co(function* doDealWithRemoteResponse () {
            ...
            let proxyReq = ( /https/.test(protocol) ? https : http).request(options, function(res) {
                co(function * () {
                    ...
                    let ifServerGzipped =  /gzip/i.test(contentEncoding);
                    if(ifServerGzipped){
...
function connectReqHandler(req, socket, head){
    ...
    co(function * () {
        ...
        async.series([
            ...
            function(callback){
            logUtil.printLog('connect handle --r');
                if(!shouldIntercept){
                    callback();
                    return;
                }else{
                    if(internalHttpsPort){
                        ...
                        getPort(function(port){
                            console.info(`getport port is: ${port} and internalHttpsPort is : ${internalHttpsPort}`);
