...
function webInterface(config){
    var port = config.port,
        ...
        ruleSummary   = "",
        customMenu    = [],
        webBasePath    = config.webBasePath || 'web2',
        server;
    const proxyInstance = config.proxyInstance;
    ...
    app.get("/latestLog",function(req,res){
        global.recorder.getRecords(null,10000,function(err,docs){
            if(err){
    ...
    app.get('/api/getInitData', function(req, res) {
        ...
        const interceptFlag = proxyInstance.getInterceptFlag();
        const globalProxyFlag = proxyInstance.getGlobalProxyFlag();
        ...
        res.json({
            ...
            port: proxyInstance.getProxyPort(),
    ...
    app.post('/api/toggleInterceptHttps', function (req, res) {
        const rootExists = certMgr.isRootCAFileExists();
        if (!rootExists) {
            certMgr.generateRootCA(function () {
    ...
    app.post('/api/toggleGlobalProxy', function (req, res) {
        const { flag } = req.body;
        let result = {};
